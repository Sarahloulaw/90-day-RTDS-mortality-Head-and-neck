WITH
	pre_provider AS(
		...
		WHERE episode.orgcodeprovider NOT IN ('7A1', '7A3', 'RQF', 'RP6')/*SL comment: RP6 moorfield eye clinic*/
        		--AND SUBSTR(episode.radiotherapydiagnosisicd,1,1) = 'C'
				--and radiotherapyintent = '02'
	),
	...
	post_provider AS(
		SELECT 
			...
			CASE
				WHEN SUBSTR(pre_provider.radiotherapydiagnosisicd,1,3) BETWEEN 'C00' AND 'C14' OR SUBSTR(pre_provider.radiotherapydiagnosisicd,1,3) BETWEEN 'C30' AND 'C32'
					THEN 'head_and_neck'
				WHEN SUBSTR(pre_provider.radiotherapydiagnosisicd,1,3) BETWEEN 'C33' AND 'C34' THEN 'lung'
				WHEN SUBSTR(pre_provider.radiotherapydiagnosisicd,1,3) ='C79' then 'C79'
							ELSE 'other'
			END cancer_group,
		...
		and radiotherapyintent = '02'
		and substr(radiotherapydiagnosisicd,1,1) = 'C'
